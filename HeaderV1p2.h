...
private:
	...
	void lookForExponents(std::string& currentEquation) {
		...
		for (int i = 0; i < currentEquation.length();) {
			if (currentEquation[i] == '^') {
				solve(currentEquation, i, '^');
				i = 0;
			}
			else {
				i++;
	...
	void solve(std::string& currentEquation, int pos, char lookFor) {
